 ...
 * Analyzes GitHub issues and comments with /triage or /check using OpenAI-compatible API.
...
async function main() {
    ...
    let isFullReportRequested = false;

    // If triggered by issue_comment
    if (isCommentTrigger) {
        const commentBody = comment.body || '';
        const hasReportCommand = /\/(triage|check)\b/i.test(commentBody);
        const hasLabelCommand = /\/label\b/i.test(commentBody);

        if (!hasReportCommand && !hasLabelCommand) {
            console.log('No /triage, /check, or /label command found in comment. Skipping.');
            return;
        }
        ...
        isFullReportRequested = hasReportCommand;
...
- Core Principles: Strictly single-currency, UUIDv7 primary keys, Result<T, Failure> error handling (no throw in repos/notifiers), strict typing (zero dynamic), CE blindness (strictly single-currency and local-first; no cloud sync or multi-currency — those belong exclusively to Poka PE).

You are provided with real source code snippets, database schema definitions, and verified repository file paths. Base your analysis directly on the real codebase, avoiding hypothetical assumptions. Everything in your output MUST be in English.

Evaluation Guidelines:
1. **Privacy & Sensitive Data Scan**:
   - Inspect the issue title and body for potential leaks of sensitive personal data (e.g. real bank account numbers, passwords, API keys, private tokens, personal identifiable data).
   - If any potential privacy leak is found, insert a warning alert at the very beginning before Section 1:
     > 🔒 **Privacy Alert**: This issue appears to contain sensitive information or credentials. Please redact or remove any personal or sensitive data immediately.

2. **Classification**:
   - Determine whether the issue is a **Bug Report** or a **Feature Proposal / Enhancement**.
   - Note on CE Blindness: If the issue requests Cloud Sync, Multi-Currency, Cloud AI, or Shared Wallets, note clearly that Poka CE is strictly single-currency and local-first. These capabilities belong to Poka PE (Premium Edition) and are out of scope for Poka CE.

3. **Section 1: Summary**:
   - Use heading: \`### 1. Summary\`.
   - 1-2 focused sentences summarizing the issue with **bold inline** technical terms.

4. **Section 2: Quality Assessment Table**:
   - Use a 3-column Markdown table: \`| Criteria | Status | Notes |\`.
   - For **Bug Reports**:
     | Criteria | Status | Notes |
     |---|---|---|
     | Steps to Reproduce | 🟢 Clear / 🟡 Partial / 🔴 Missing | [Brief evaluation] |
     | Expected vs Actual | 🟢 Clear / 🟡 Partial / 🔴 Missing | [Brief evaluation] |
     | Environment / Logs | 🟢 Clear / 🟡 Partial / 🔴 Missing | [Brief evaluation] |
     | Overall Verdict | 🚀 Ready to Investigate / ⚠️ Needs More Info | [Brief conclusion] |
   - For **Feature Proposals / Enhancements**:
     | Criteria | Status | Notes |
     |---|---|---|
     | Problem & Use Case | 🟢 Clear / 🟡 Partial / 🔴 Missing | [Brief evaluation] |
     | Technical Feasibility | 🟢 Feasible / 🟡 Complex / 🔴 Unfeasible | [Brief evaluation] |
     | Scope & CE Alignment | 🟢 Aligned / ⛔ Out of Scope (PE only) | [Brief evaluation] |
     | Overall Verdict | 💡 Ready to Plan / ⚠️ Needs Refinement / ⛔ Out of Scope | [Brief conclusion] |

5. **Section 3: Technical Analysis & Affected Areas**:
   - Use heading: \`### 3. Technical Analysis & Affected Areas\`.
   - List **Suspected Feature Area** and **Target Files** (format file paths in inline code backticks, referencing ONLY real files that exist in the repository lib/ directory).
   - Wrap the technical hypothesis or suggested implementation approach in a Markdown blockquote (\`>\`).

6. **Section 4: Action Items**:
   - Use heading: \`### 4. Action Items\`.
   - Include two sub-headings: \`#### 👤 For Reporter\` and \`#### 🛠️ For Maintainer / Developer\`.
   - Use GitHub task list format (\`- [ ]\`) for actionable items.

7. **Section 5: Suggested Labels**:
   - Use heading: \`### 5. Suggested Labels\`.
   - Display suggested labels as inline code chips (e.g. \`bug\` \`transactions\`).
   - Allowed labels whitelist: "bug", "enhancement", "question", "needs-info", "database", "accounts", "budgets", "categories", "debts", "goals", "recurring", "transactions", "ui".
   - At the very end of your response on its own line, output the machine marker:
...
${fileCatalogSample}`;
    ...
    const baseUrl = (process.env.OPENAI_BASE_URL || 'https://aig.octopy.dev/compat').trim().replace(/\/+$/, '');
    const rawModel = (process.env.OPENAI_MODEL || process.env.AI_MODEL || 'dynamic/copilot').trim();
    const model = rawModel.replace(/^openai\//i, '');

    console.log(`Calling OpenAI API at ${baseUrl}/chat/completions using model "${model}"...`);
    const response = await fetch(`${baseUrl}/chat/completions`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${apiKey}`,
            'HTTP-Referer': 'https://aig.octopy.dev',
            'X-OpenRouter-Title': 'Octopy AIG',
            'X-Title': 'Octopy AIG',
    ...
    if (isFullReportRequested) {
        ...
        console.log('Silent / label-only mode: skipping detailed comment posting.');
    }

    // Acknowledge command comment with reaction
    if (isCommentTrigger) {
