...
class TransactionTypeSwitcher extends StatelessWidget {
  ...
  const new({required this.selectedType, required this.onChanged, this.disabled = false, super.key});
  ...
  Widget build(BuildContext context) {
    return IgnorePointer(
      ...
      child: Opacity(
        ...
        child: FTabs(
          control: FTabControl.lifted(index: _types.indexOf(selectedType), onChange: (idx) => onChanged(_types[idx])),
          children: _types.map((type) {
            ...
            return FTabEntry(label: Text(label), child: const SizedBox.shrink());
          }).toList(),
        ),
      ),
    );
