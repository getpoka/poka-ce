...
| **BUG-007** | `Transactions / Localization & Formatting` | Incomplete translations on Transactions page: unlocalized date headers ("Wed, 10 Sep"), English month labels ("September 2026"), and hardcoded creation form tabs ("Income", "Expense", "Transfer") | High (Localization & UX Quality) | 🟢 Solved ([#42](https://github.com/getpoka/poka-ce/issues/42)) |
...
4. **Default Categories:** Seeded categories in SQLite (`Food & Dining`, `Groceries`, `Transport`, `Housing`, etc.) are English-only and do not translate dynamically based on active locale (tracked separately as a schema/seed enhancement).
...
15. [x] Implement Notification Permission Rationale Sheet and manual test trigger for Backup Reminder (BUG-015 — [#49](https://github.com/getpoka/poka-ce/issues/49)).
